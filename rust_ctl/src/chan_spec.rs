        ...
        match self {
            ChanSpecGeneric::Each(vals) => {
                ...
                Ok(chans
                    .iter()
                    .zip(vals)
                    .map(|(cdesc, val)| (cdesc.chan_id, *val))
                    .collect())
            }
...
impl ChanSpec {
    ...
    pub fn parse_f32(string: &str) -> Result<ChanSpec, String> {
        let parsed_parts: Vec<(Option<&str>, f32)> = string
            .split(',')
            .map(|p| {
                ...
                let chanval: (Option<&str>, f32) = match chanval.len() {
                    ...
                    _ => Err(format!(
                        "Invalid argument '{}', expected CHAN:VAL or VAL",
                        p
                    )),
                }?;
                Ok(chanval)
        ...
        match parsed_parts.len() {
            ...
            1 => {
                ...
                if chan.is_some() {
                    Ok(ChanSpec::F32(ChanSpecGeneric::Some(vec![(
                        chan.unwrap().to_string(),
                        val,
                    )])))
                } else {
                    Ok(ChanSpec::F32(ChanSpecGeneric::SomeWithDefault(
                        val,
                        Vec::new(),
                    )))
                }
            ...
            _ => {
                // we know we have at least 2 items
                if parsed_parts[0].0.is_some() {
                    ...
                    return Ok(ChanSpec::F32(ChanSpecGeneric::Some(
                        parsed_parts
                            .iter()
                ...
                if parsed_parts[1].0.is_some() {
                    // SomeWithDefault
                    if !parsed_parts
                        .iter()
                        .skip(1)
                        .all(|(chan, _val)| chan.is_some())
                    {
                        return Err("If the second value specifies chan_id\
                                    then all the rest of them should"
                            .to_string());
                    }

                    let (_, default_val) = parsed_parts[0];
                    return Ok(ChanSpec::F32(ChanSpecGeneric::SomeWithDefault(
                        default_val,
                        parsed_parts
                            .iter()
                            .skip(1)
                ...
                return Ok(ChanSpec::F32(ChanSpecGeneric::Each(
